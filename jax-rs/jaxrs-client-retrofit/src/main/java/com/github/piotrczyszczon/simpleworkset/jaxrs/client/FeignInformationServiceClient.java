...
public class FeignInformationServiceClient
{
  ...
  private void run()
  {
    ...
    InformationService annotationBasedService = new RetrofitServiceClient(new AnnotationServicePathProvider()).openService();
    testService(annotationBasedService);

    System.out.println("Testing [CONFIGURATION] BASED Service!");
    InformationService configurationBasedService = new RetrofitServiceClient(new ConfigurationServicePathProvider()).openService();
